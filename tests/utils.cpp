...
std::unique_ptr<DictionaryExpr> Factories::create_cfg_dict(std::unique_ptr<Expr> compiler,
    ...
    cmap[ConfigFactory::COMPILER_FIELD] = std::move(compiler);
    ...
    cmap[ConfigFactory::COMPILATION_FLAGS_FIELD] = std::make_unique<ListExpr>(std::move(c_flags));
    ...
    cmap[ConfigFactory::LINK_FLAGS_FIELD] = std::make_unique<ListExpr>(std::move(l_flags));

    cmap[ConfigFactory::DEFAULT_FIELD] = std::make_unique<StringExpr>(std::move(default_rule));
